...
export const apiGetListCourse = async (props: {
  page: number;
  pageSize?: number;
  MaNhom?: string;
}) => {
  ...
  try {
    const response = await fetch(
      `https://elearningnew.cybersoft.edu.vn/api/QuanLyKhoaHoc/LayDanhSachKhoaHoc_PhanTrang?page=${page}&pageSize=12&MaNhom=GP01`,
      {
        method: 'GET',
        headers: {
          Tokencybersoft: `${token}`,
...
export const apiFetchCourseByCategory = async (category: string) => {
  try {
    const response = await fetch(
      `${BASE_URL}/QuanLyKhoaHoc/LayKhoaHocTheoDanhMuc?maDanhMuc=${category}&MaNhom=GP01`,
      {
        method: 'GET',
        headers: {
          Tokencybersoft: `${token}`,
...
export const apiDetailCourse = async (maKhoaHoc: string) => {
  ...
  try {
    const response = await fetch(
      `${BASE_URL}/QuanLyKhoaHoc/LayThongTinKhoaHoc?maKhoaHoc=${maKhoaHoc}`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
